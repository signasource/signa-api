...
public class FirebaseService {
	...
	public FcmResult sendToTokens(List<String> tokens, PushMessage message) {
		...
		MulticastMessage multicast = MulticastMessage.builder().addAllTokens(tokens)
				.setNotification(toNotification(message)).putAllData(dataOrEmpty(message)).build();
		...
		try {
			...
			log.info("FCM multicast: {} requested, {} ok, {} failed, {} stale token(s) to purge", tokens.size(),
					response.getSuccessCount(), response.getFailureCount(), invalidTokens.size());
			return FcmResult.of(response.getSuccessCount(), response.getFailureCount(), invalidTokens);
		} catch (FirebaseMessagingException e) {
			log.error("FCM multicast to {} token(s) failed entirely (errorCode={})", tokens.size(),
	...
	public void sendToTopic(String topic, PushMessage message) {
		Message msg = Message.builder().setTopic(topic).setNotification(toNotification(message))
				.putAllData(dataOrEmpty(message)).build();

		try {
			String messageId = firebaseMessaging.send(msg);
			log.info("FCM topic '{}' message sent (id={})", topic, messageId);
		} catch (FirebaseMessagingException e) {
			log.error("FCM message to topic '{}' failed (errorCode={})", topic, e.getMessagingErrorCode(), e);
		}
	}
	...
	public void subscribeToTopic(List<String> tokens, String topic) {
		...
		try {
			...
			log.error("Failed to subscribe {} token(s) to FCM topic '{}' (errorCode={})", tokens.size(), topic,
					e.getMessagingErrorCode(), e);
		}
	}
	...
	public void unsubscribeFromTopic(List<String> tokens, String topic) {
		...
		try {
			...
			log.error("Failed to unsubscribe {} token(s) from FCM topic '{}' (errorCode={})", tokens.size(), topic,
					e.getMessagingErrorCode(), e);
		}
	}
	...
	private List<String> collectStaleTokens(List<String> tokens, BatchResponse response) {
		...
		for (int i = 0; i < responses.size(); i++) {
			...
			if (isStaleTokenError(errorCode)) {
				...
				log.warn("FCM rejected token {} with errorCode={}; keeping the token", mask(tokens.get(i)), errorCode);
			}
		}
		return staleTokens;
	}
	...
	private boolean isStaleTokenError(MessagingErrorCode errorCode) {
		return errorCode == MessagingErrorCode.UNREGISTERED || errorCode == MessagingErrorCode.SENDER_ID_MISMATCH;
	}
