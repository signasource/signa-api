...
public class MatchValidator implements BlockValidator {
    ...
    public void validate(LessonBlockDto block, ValidationContext ctx, List<String> errors) {
        ...
        if (parsed.isEmpty()) {
            errors.add(ctx.location() + ": invalid config for MATCH block");
            return;
        }
        MatchConfig config = parsed.get();

        if (config.concepts() == null) {
            errors.add(ctx.location() + ": concepts is required");
        } else if (config.concepts().size() < 2) {
            errors.add(ctx.location() + ": concepts must have at least 2 elements");
        }
    }
}
