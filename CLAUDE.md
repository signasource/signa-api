...
| `gamification` | Stats de usuario, logros, desafíos, tienda, compras, regalos; XP diario/semanal, señas aprendidas y mecánica de vidas/racha. **El catálogo de la tienda se siembra en un único lugar:** `resources/db/seed/store_items.sql` (lo corre `common/DataLoader`), que hace upsert por `code` — así editar un precio o un texto se aplica al reiniciar. No agregues un `CommandLineRunner` que siembre `shop_items`: hubo uno y duplicaba el catálogo. Los textos de ese seed son user-facing, así que van **en español** (única excepción a la regla de §3) |
