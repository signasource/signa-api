...
| `learning` | Cursos, versiones, temas, lecciones, bloques, señas y reportes de señas; seguimiento de progreso (inscripciones, progreso de tema/lección, intentos de bloque); animación de seña vía presigned URL de R2 (`GET /signs/{meaning}/animation`; `meaning` es único) y su variante batch (`POST /signs/animations`, usada por el lesson player para precargar todas las animaciones de una lección en un solo request). **Tipos de bloque:** `INFO` (contenido textual, opcionalmente con `myths`), `INTRODUCE_SIGN` (presenta una seña nueva; campos `meaning` + `word`), `SELECT_MEANING`, `SELECT_SIGN`, `CONTEXT_RESPONSE`, `MATCH`, `VISUAL_RECOGNITION`, `INVISIBLE_SIGNS` (no se muestra al usuario; campo `signs`: lista de meanings que se insertan en el catálogo de señas durante el import — útil para precargar señas de lecciones que no usan `INTRODUCE_SIGN`). **Práctica libre** (`PracticeController`/`PracticeService`, `/practice/*`): sesiones sueltas de repaso por tipo de ejercicio, por seña aprendida (`UserLearnedSign`) o de errores pendientes, reutilizando los `LessonBlock` de los cursos en los que el usuario está inscripto. Los intentos se guardan en `PracticeAttempt` — deliberadamente separada de `LessonBlockAttempt`: no dispara XP, vidas ni progreso de lección/tema/curso. El repaso de errores combina ambas tablas (ver [docs/diagrams/sequence.md](docs/diagrams/sequence.md#practica-libre)). Excepción: completar un lote de repaso de errores (`POST /practice/mistakes/complete`) sí otorga una recompensa de XP fija (`PracticeService.completeMistakeReview`, publica `XpEarnedEvent`) — se autolimita porque siempre opera sobre errores pendientes reales |
...
| `gamification` | Stats de usuario, logros, desafíos, tienda, compras, regalos; XP diario/semanal, señas aprendidas y mecánica de vidas/racha. **Packs de gemas con dinero real** vía Google Play (`GemPack` = producto in-app de Play + cantidad de gemas, sembrado por `GemPackSeeder`; `GET /store/gem-packs`). El cliente compra en Play y manda `{productId, purchaseToken}` a `POST /store/gem-purchases`: `GemPurchaseService` verifica el token contra la Google Play Developer API (`GooglePlayPurchaseVerifier`, fuera de la transacción) y `GemCreditService` acredita las gemas y persiste `GemPurchase` con `purchaseToken` **único** (idempotente: repetir el token del mismo usuario devuelve `alreadyGranted=true`; de otro usuario o ya consumido → 409). Los ítems `GEMS` del catálogo con precio en gemas quedan inactivos (`store_items.sql`). Las gemas **nunca** se acreditan sin verificación del servidor |
...
- **Rate limiting** con Bucket4j (`RateLimitInterceptor`) en endpoints sensibles a abuso
  (`/users/username-availability`, `/store/gem-purchases`).
...
- **Secretos** por variables de entorno (`.env`, ver `.env.example`): `JWT_SECRET`,
  ...
  Cloudflare R2 (`R2_ACCESS_KEY_ID`/`R2_SECRET_ACCESS_KEY`), service account de Google Play
  (`GOOGLE_PLAY_CREDENTIALS_JSON`, base64 del JSON, con permisos *View financial data* + *Manage
  orders* en Play Console). Nunca commitear secretos.
